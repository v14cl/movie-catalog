...
public class MovieDao {
    ...
    private static final String INSERT_SQL = "INSERT INTO movies (title, genre, rating, author_id) VALUES (?, ?, ?, ?)";
    private static final String SELECT_LAST_ID_SQL = "SELECT last_insert_rowid()";
    private static final String SELECT_BY_ID_SQL = "SELECT m.*, a.name FROM movies m JOIN authors a ON m.author_id = a.id WHERE m.id = ?";
    ...
    public RecommendGenerator streamRecommendedMovies() throws SQLException {
        PreparedStatement stmt = connection.prepareStatement(GET_RECOMM_SQL);
        ResultSet rs = stmt.executeQuery();
        return new RecommendGenerator(stmt, rs);
    }
